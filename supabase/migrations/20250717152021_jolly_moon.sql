/*
  # Complete VidGro Database Setup - Error Fix
  
  This migration sets up the entire VidGro database from scratch with proper error handling
  and ensures all components work correctly for user signup and app functionality.

  1. Extensions
    - Enable all required PostgreSQL extensions
    - Handle cases where extensions might not be available
  ...
  4. Functions
    - Create all required functions with proper error handling
    - Alternative implementations for missing extensions

  5. Triggers
    - Set up user creation trigger with robust error handling
...
DO $$
BEGIN
    -- Try to enable pgcrypto for gen_random_bytes
    BEGIN
        CREATE EXTENSION IF NOT EXISTS "pgcrypto";
    EXCEPTION
        WHEN OTHERS THEN
            RAISE NOTICE 'pgcrypto extension not available, will use alternative methods';
    END;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE NOTICE 'uuid-ossp extension not available, will use gen_random_uuid()';
    END;
END $$;
...
CREATE OR REPLACE FUNCTION safe_gen_uuid()
RETURNS uuid
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Safe random bytes generation
CREATE OR REPLACE FUNCTION safe_gen_random_bytes(length integer)
RETURNS bytea
LANGUAGE plpgsql
AS $$
BEGIN
    -- Try gen_random_bytes first
    BEGIN
        RETURN gen_random_bytes(length);
    EXCEPTION
        WHEN OTHERS THEN
            -- Fallback: generate random bytes using md5 and random()
            RETURN decode(
                substring(
...
BEGIN
    LOOP
        ...
        -- Generate 8-character alphanumeric code using multiple methods
        BEGIN
            -- Method 1: Use safe_gen_random_bytes
            code := upper(encode(safe_gen_random_bytes(4), 'hex'));
        EXCEPTION
            WHEN OTHERS THEN
                -- Method 2: Use md5 hash of random values
                code := upper(substring(md5(random()::text || clock_timestamp()::text) from 1 for 8));
        END;
        
        -- Ensure code is exactly 8 characters and alphanumeric
        code := regexp_replace(code, '[^A-Z0-9]', '', 'g');
        IF length(code) < 8 THEN
            code := code || upper(substring(md5(random()::text) from 1 for 8 - length(code)));
        END IF;
        ...
        IF NOT exists_check OR attempt_count > 10 THEN
            EXIT;
        END IF;
    END LOOP;
    ...
    IF exists_check THEN
        code := substring(code from 1 for 4) || to_char(extract(epoch from now())::integer % 10000, 'FM0000');
    END IF;
...
CREATE TABLE IF NOT EXISTS videos (
    id uuid PRIMARY KEY DEFAULT safe_gen_uuid(),
        ...
        EXCEPTION
            WHEN unique_violation THEN
                ...
                IF attempt_count < max_attempts THEN
                    ...
                    referral_code_value := generate_referral_code();
                ELSE
                    -- After max attempts, use a guaranteed unique username
...
-- Update any existing profiles that might have missing referral codes
DO $$
DECLARE
